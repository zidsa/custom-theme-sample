...
class SlideMenu {
  ...
  public toggle(show?: boolean, animate: boolean = true): void {
    ...
    if (animate) {
      this.moveSlider(this.menuElem, offset, true);
    } else {
      const action = this.moveSlider.bind(this, this.menuElem, offset, true);
   ...
   */
  private moveSlider(elem: HTMLElement, offset: string | number, ignoreRightFix?: boolean): void {
    // Add percentage sign
      ...
      if(this.options.position === MenuPosition.Right && !ignoreRightFix){
          offset = offset * -1;
      }
